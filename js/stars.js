...
function startStars(){
  ...
  for(let i=0;i<TOTAL_STARS;i++){
    ...
    star.style.left=(5+Math.random()*90)+"%"; star.style.top=(5+Math.random()*82)+"%";
...
function collectStar(star){
  ...
  star.dataset.collected="1"; collectedStars++;
  star.style.transform="scale(2)"; star.style.opacity="0"; star.style.pointerEvents="none";
...
function createHeartRain(){
  ...
  c.innerHTML="";
  for(let i=0;i<18;i++){
    const el=document.createElement("div"); el.className="heart"; el.textContent="❤️";
    el.style.left=Math.random()*100+"%"; el.style.animationDelay=Math.random()*2+"s"; c.appendChild(el);
  }
}
...
function createRosePetals(){
  ...
  c.innerHTML="";
  for(let i=0;i<12;i++){
    const el=document.createElement("div"); el.className="petal"; el.textContent="🌹";
    el.style.left=Math.random()*100+"%"; el.style.animationDelay=Math.random()*2+"s"; c.appendChild(el);
  }
}
...
function startFutureStory(){
  ...
  if(!box) return;
  futureIndex=0; box.textContent=""; if(next) next.classList.add("hidden"); if(img) img.style.display="none";
  typeFutureMessage();
}
...
function typeFutureMessage(){
  ...
  box.textContent+=futureMessage.charAt(futureIndex++); setTimeout(typeFutureMessage,35);
}
...
function startFinalLetter(){
  ...
  if(!box) return;
  finalIndex=0; box.textContent=""; if(btn) btn.classList.add("hidden"); typeFinalLetter();
}
...
function typeFinalLetter(){
  ...
  if(finalIndex>=finalMessage.length){ const btn=document.getElementById("sealChapterBtn"); if(btn) btn.classList.remove("hidden"); return; }
  box.textContent+=finalMessage.charAt(finalIndex++); setTimeout(typeFinalLetter,35);
}
...
function createFireworks(){
  ...
  c.innerHTML="";
  for(let i=0;i<14;i++){ const e=document.createElement("div"); e.className="firework"; e.style.left=(15+Math.random()*70)+"%"; e.style.top=(10+Math.random()*60)+"%"; c.appendChild(e); }
}

function startSealSequence(){
  createFireworks(); createHeartRain();
