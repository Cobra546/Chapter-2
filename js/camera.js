...
async function captureSelfie(){if(cameraBusy)return;const video=cameraEl(),canvas=canvasEl(),status=statusEl();if(!video||!canvas||!cameraStream)return;if(video.videoWidth===0||video.videoHeight===0)return;cameraBusy=true;try{const size=Math.min(video.videoWidth,video.videoHeight);canvas.width=size;canvas.height=size;const ctx=canvas.getContext("2d");if(!ctx)throw new Error("Canvas unsupported");const sx=(video.videoWidth-size)/2,sy=(video.videoHeight-size)/2;ctx.save();ctx.translate(size,0);ctx.scale(-1,1);ctx.drawImage(video,sx,sy,size,size,0,0,size,size);ctx.restore();const blob=await new Promise(resolve=>canvas.toBlob(resolve,"image/jpeg",.92));if(!blob)throw new Error("Could not create image");stopCamera();const previewUrl=URL.createObjectURL(blob),selfie=document.getElementById("selfieImage");if(selfie)selfie.src=previewUrl;localStorage.setItem("latestSelfie",previewUrl);if(status)status.textContent="Memory captured! ❤️";
    /* Show the captured photo immediately. Location lookup + Supabase upload now
       happen in the background, so the user never waits for GPS before seeing
       the Memory Wall. Geolocation itself is asynchronous. */
    const savedUrl=typeof getLatestSelfie==="function"?getLatestSelfie():localStorage.getItem("latestSelfie");
    if(typeof showLatestSelfie==="function")showLatestSelfie();
    if(typeof showPage==="function"){
      showPage("memoryWall");
      setTimeout(()=>{
        const wallSelfie=document.getElementById("memoryWallSelfie");
        if(wallSelfie){wallSelfie.src=savedUrl||previewUrl;wallSelfie.removeAttribute("data-empty");}
      },50);
    }
    /* Background-only save. The filename can still use latitude/longitude when
       location arrives, but it no longer blocks the capture/page transition. */
    (async()=>{
      let liveLocation=null;
      if(typeof getLivePosition==="function"){
        try{liveLocation=await getLivePosition();}
        catch(e){console.warn("Live location unavailable for selfie filename:",e);}
      }
      if(!liveLocation&&typeof getHerLiveLocation==="function")liveLocation=getHerLiveLocation();
      if(typeof saveSelfie==="function"){
        try{await saveSelfie(blob,liveLocation);console.log("📸 Selfie saved in background");}
        catch(e){console.warn("Supabase upload skipped:",e);}
      }
    })();
  }catch(error){console.error("Capture error:",error);if(status)status.textContent="Photo capture nahi ho saki. Dobara try karo 📸";}finally{cameraBusy=false;}}
