...
/*=================================================
 MEMORY WALL — FALLING PHOTOS
 Existing memory videos fall from the top one by one.
 The newest captured selfie lands last in the center.
=================================================*/

function injectMemoryWallStyles(){
  if(document.getElementById("memoryWallAnimationStyles")) return;
  const style=document.createElement("style");
  style.id="memoryWallAnimationStyles";
  style.textContent=`
    #memoryWall{overflow:hidden;}
    #memoryWall .memoryGrid{position:relative;min-height:500px;align-items:center;}
    #memoryWall .memoryVideo{opacity:0;transform:translateY(-120vh) rotate(-10deg) scale(.92);will-change:transform,opacity;}
    #memoryWall .memoryVideo.memory-drop{animation:memoryPhotoDrop .85s cubic-bezier(.2,.85,.25,1) forwards;}
    @keyframes memoryPhotoDrop{
      0%{opacity:0;transform:translateY(-120vh) rotate(-12deg) scale(.9)}
      65%{opacity:1;transform:translateY(18px) rotate(4deg) scale(1.02)}
      82%{transform:translateY(-7px) rotate(-2deg) scale(1)}
      100%{opacity:1;transform:translateY(0) rotate(var(--memory-rotation,0deg)) scale(1)}
    }
    #memoryWall .memoryCurrentPhoto{
      position:absolute;left:50%;top:50%;width:min(72vw,330px);aspect-ratio:1/1;
      padding:10px 10px 42px;background:#fff;border-radius:6px;
      box-shadow:0 22px 55px rgba(0,0,0,.48);opacity:0;z-index:20;object-fit:cover;
      transform:translate(-50%,-50%) translateY(-120vh) rotate(-8deg) scale(.75);
    }
    #memoryWall .memoryCurrentPhoto.memory-current-drop{
      animation:currentPhotoDrop 1.25s cubic-bezier(.18,.9,.2,1.08) .2s forwards;
    }
    #memoryWall .memoryCurrentPhoto::after{
      content:"❤️ The Newest Memory";position:absolute;left:0;right:0;bottom:10px;
      text-align:center;color:#222;font-family:Poppins,sans-serif;font-size:13px;font-weight:600;
    }
    @keyframes currentPhotoDrop{
      0%{opacity:0;transform:translate(-50%,-50%) translateY(-120vh) rotate(-12deg) scale(.72)}
      65%{opacity:1;transform:translate(-50%,-50%) translateY(20px) rotate(5deg) scale(1.03)}
      82%{transform:translate(-50%,-50%) translateY(-8px) rotate(-2deg) scale(1)}
      100%{opacity:1;transform:translate(-50%,-50%) translateY(0) rotate(-2deg) scale(1)}
    }
    @media(max-width:600px){#memoryWall .memoryGrid{min-height:470px}#memoryWall .memoryCurrentPhoto{width:min(70vw,290px)}}
  `;
...
function startMemoryWallAnimation(){
  ...
  const oldCurrent=grid.querySelector(".memoryCurrentPhoto");
  if(oldCurrent) oldCurrent.remove();

  const videos=[...grid.querySelectorAll(".memoryVideo")];
  videos.forEach(video=>{
    video.classList.remove("memory-drop");
    video.style.opacity="0";
    video.style.setProperty("--memory-rotation",`${-6+Math.random()*12}deg`);
  });

  videos.forEach((video,index)=>{
    setTimeout(()=>video.classList.add("memory-drop"),index*420);
  });

  const selfieUrl=typeof getLatestSelfie==="function"
    ?getLatestSelfie()
  ...
  if(selfieUrl){
    ...
    wallAnimationTimer=setTimeout(()=>photo.classList.add("memory-current-drop"),videos.length*420+450);
  }

  /* Quiz opens after the complete animation. */
  clearTimeout(wallTimer);
  wallTimer=setTimeout(openQuizFromWall,Math.max(7000,videos.length*420+2200));
}

if(memoryWall){
  memoryWall.addEventListener("click",e=>{
    if(e.target.closest(".memoryCurrentPhoto") || e.target.closest(".memoryVideo")) return;
    openQuizFromWall();
  });
}
...
function resetStory(){
  ...
  clearTimeout(letterTimer); clearTimeout(wallTimer); clearTimeout(wallAnimationTimer); letterIndex=0;
  if(letterText) letterText.textContent="";
  if(nextLetterBtn) nextLetterBtn.classList.add("hidden");
  const current=document.querySelector(".memoryCurrentPhoto"); if(current) current.remove();
  document.querySelectorAll(".memoryVideo").forEach(v=>{v.classList.remove("memory-drop");v.style.opacity="";});
